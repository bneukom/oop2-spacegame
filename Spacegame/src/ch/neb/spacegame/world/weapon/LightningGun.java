	...
	protected void doShoot(Vec2 position, Vec2 direction) {
		...
		if (target == null) {
			...
			for (GameEntity entity : world.getGameEntities()) {
				...
				// TODO hardcoded player check is not good!
		...
		if (targetToHit != null) {
			spawnLightning(position, targetToHit);
		}

	}

	private void spawnLightning(Vec2 position, Mob targetToHit) {
		world.addEntity(new Lightning(world, new Vec2(position), new Vec2(targetToHit.getPosition()).translate(targetToHit.getWidth() / 2, targetToHit.getHeight() / 2), 0,
				false));
		world.addEntity(new Lightning(world, new Vec2(position), new Vec2(targetToHit.getPosition()).translate(targetToHit.getWidth() / 2, targetToHit.getHeight() / 2), 0,
				true));
		world.addEntity(new Lightning(world, new Vec2(position), new Vec2(targetToHit.getPosition()).translate(targetToHit.getWidth() / 2, targetToHit.getHeight() / 2), 0,
		...
		targetToHit.doDamage(owner, damage);
	}
